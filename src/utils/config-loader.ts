...
export class ConfigLoader {
   ...
   */
  private static loadExplicitConfig(projectRoot: string): ResolverConfig | null {
   ...
   * Load tsconfig.json
   */
  private static loadTsConfig(projectRoot: string): TsConfigPaths | null {
   ...
   */
  private static loadJsConfig(projectRoot: string): TsConfigPaths | null {
   ...
   */
  private static extractTsConfigAliases(
   ...
   */
  private static loadPythonPaths(projectRoot: string): string[] {
    // Try pytest.ini
  ...
  private static parsePytestIni(filePath: string): string[] {
    try {
  ...
  private static parseSetupCfg(filePath: string): string[] {
    try {
      const content = fs.readFileSync(filePath, 'utf-8');
  ...
  private static parsePyprojectToml(filePath: string): string[] {
    try {
      const content = fs.readFileSync(filePath, 'utf-8');
   ...
   */
  private static normalizeConfig(config: any, projectRoot: string): ResolverConfig {
    const normalized: ResolverConfig = {
      aliases: {},
